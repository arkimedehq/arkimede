...
This product (**Arkimede**) includes third-party software.
The required attributions and license notices are listed below.
...
> Note: Arkimede vendors its own **clean-room** NTLM implementation at
...
|---|---|
| MIT, ISC, 0BSD, Unlicense, CC0 | Retain copyright and license text |
| BSD-2-Clause / BSD-3-Clause | Retain copyright; BSD-3 forbids endorsement |
| Apache-2.0 | Retain copyright, NOTICE and changelog of modified files; patent grant |

---

## Specific license notes

### Apache License 2.0 — relevant components

The following direct packages are under Apache-2.0. Apache-2.0 requires retaining
the copyright notices and any original `NOTICE` files upon redistribution, and grants
...
## Complete list of Node packages (by license)

Packages deduplicated by `name@version` across the `backend`, `frontend`,
`bridge`, `executor` workspaces. The `applescript` and `pause` packages are reported as
MIT, verified from the LICENSE file / the author's common practice (the `license` field
is absent from their `package.json`).
