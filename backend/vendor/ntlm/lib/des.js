/*
 * Pure-JavaScript DES (ECB, single 8-byte block) implementation.
 *
 * Self-contained: does NOT depend on node:crypto or any OpenSSL provider.
...
// Turn an 8-byte buffer into an array of 64 bits (MSB first).
function bytesToBits(buf) {
  var bits = new Array(buf.length * 8);
...
// Turn an array of 64 bits (MSB first) into an 8-byte buffer.
function bitsToBytes(bits) {
  var out = Buffer.alloc(bits.length / 8);
...
function generateSubkeys(keyBits) {
  var permuted = permute(keyBits, PC1); // 56 bits
  ...
  for (var round = 0; round < 16; round++) {
    ...
    subkeys.push(permute(C.concat(D), PC2)); // 48 bits
  }
  return subkeys;
}

function feistel(R, subkey) {
  var expanded = permute(R, E); // 48 bits
  var xored = new Array(48);
  for (var i = 0; i < 48; i++) {
  ...
  for (var box = 0; box < 8; box++) {
    var offset = box * 6;
    var row = (xored[offset] << 1) | xored[offset + 5];
    var col =
      (xored[offset + 1] << 3) |
      (xored[offset + 2] << 2) |
      (xored[offset + 3] << 1) |
      xored[offset + 4];
    var val = SBOX[box][row * 16 + col];
    output[box * 4] = (val >> 3) & 1;
    output[box * 4 + 1] = (val >> 2) & 1;
    output[box * 4 + 2] = (val >> 1) & 1;
    output[box * 4 + 3] = val & 1;
  }
 ...
 *
 * key   : 8-byte Buffer (parity-adjusted, as produced by expandkey/oddpar)
 * block : 8-byte Buffer of plaintext
 * returns: 8-byte Buffer of ciphertext
 */
function desEncryptBlock(key, block) {
  var keyBits = bytesToBits(key);
  var subkeys = generateSubkeys(keyBits);

  var bits = permute(bytesToBits(block), IP);
  var L = bits.slice(0, 32);
  var R = bits.slice(32, 64);

  for (var round = 0; round < 16; round++) {
    var f = feistel(R, subkeys[round]);
  ...
  // Note: after the 16th round the halves are swapped (R then L).
  var preOutput = R.concat(L);
  return bitsToBytes(permute(preOutput, FP));
}

module.exports.desEncryptBlock = desEncryptBlock;
